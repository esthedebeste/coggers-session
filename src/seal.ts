...
export async function seal(
	...
	const keyE = await pbkdf2(password, saltE, 1, 32, "sha1");

	const cipher = crypto.createCipheriv("aes-256-cbc", keyE, iv);
	const dataE = Buffer.concat([cipher.update(data), cipher.final()]);

	// integrity
	const saltI = crypto.randomBytes(256);
	const keyI = await pbkdf2(password, saltI, 1, 32, "sha1");
...
export async function unseal(
	...
	if (isNaN(passid) || passid >= passwords.length)
